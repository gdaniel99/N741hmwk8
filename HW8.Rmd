...
Mitoses is the clear outlier that does not lie within the cluster that includes the other variables.
...
The cluster of variables (excluding Mitoses) is a tighter, closer cluster in the Benign subset loadings plot than in the Malignant. The Malignant subset loadings plot appears to have a loose cluster composed of all the variables except clumpthickness and barenucfix; however, neither of the two appear to be exteme outliers. The blandchromatin and marginaladhesion variables appear to cluster, and the uniformcellsize and uniformcellshape variables appear to cluster.
...
2 PC appears to be reasonable for the overall dataset, the Benign subset, and the Malignant malignant subset. In all the groups, 1 PC appears to account for almost all the variability, followed by a seemingly significant decrease in variability at 2 PCs, and a gradual tapering off in variability as the number of PCs increase. 2 PCs is not as extreme as 1, yet it accounts for 1 without neglecting to account for the smaller variability seen in 3+ PCs, which makes it reasonable for summarizing the variability in the groups.
...
When Benign and Malignant cases are ran together, the 2 PCs extracted appear to do a good job helping to distinguish as evidenced by the clustering of data with little overlap on the Scores Plot.
